...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *content = NULL;
    ...
    if (file != NULL)
    {
        ...
        for(int j = fread(tempbuff, 1, BYTES, file); j != 0; j = fread(tempbuff, 1, BYTES, file)){
            
            *content = realloc(*content, 1 + *length + j);
            memcpy(*length + *content, tempbuff, j);
            *length = j + *length;
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // TODO #2    method SP request-target SP HTTP-version CRLF
    char request_line[strlen(line)];
    strcpy(request_line, line);

    //405   if method is not GET, respond to the browser with 405 Method Not Allowed and return false
    char* method = strtok(request_line, " "); // [method SP request-target SP HTTP-version CRLF]
    if(strcmp(method, "GET") != 0){
        error(405);
        return false;   
    }
    
    //501   if request-target does not begin with /, respond to the browser with 501 Not Implemented and return false;
    //char* request_target = strtok(request_line, " "); // method SP [request-target SP HTTP-version CRLF]
    ...
    //400   if request-target contains a ", respond to the browser with 400 Bad Request and return false;
    for(int q = 0; q < strlen(request_target); q++){
        if(request_target[q] == '"'){
            error(400);
            return(false);
        }        
    ...
    //505  if HTTP-version is not HTTP/1.1, respond to the browser with 505 HTTP Version Not Supported and return false; or
    char* HTTP_version = strtok(NULL, " "); // method SP request-target SP [HTTP-version CRLF]
    if(strcmp(HTTP_version, "HTTP/1.1\r\n") != 0){
        error(505);
        return(false);   
    }

    //abs_path  absolute-path (which will not contain ?) 
    for(int p = 0; request_target[p] != '?' && request_target[p] != ' '; p++){
       abs_path[p] = request_target[p];
    }
    
    //query which may not contain ".
